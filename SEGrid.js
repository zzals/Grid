...
class SEGrid{
	...
	gridMenu (){
		...
		* Grid Opttions
		*/
	   if(setting.option.use){
	   
			// Option Objects
		   if(setting.option.objects.length > 0){
	...
	gridHead (json){
				...
				// 일반적인 서브 컬럼
					...
					for(var j=0; j<sub_len; j+=1){	
						...
						//	sub_caption = gridObj.madeLanguage.langCaption(subCaptions);
							...
							if(setting.height === "none" || setting.height == 0){
								if( i+j == len-1){
		...
		if(setting.height === "auto" || setting.height === "dash" || setting.height > 0){
	...
	gridBody (json){	
		...
		for(var i=0; i < this.colSize; i+=1){
		...
		if(this.setting.height === "auto" || this.setting.height > 0){
			var col = document.createElement('col');
			col.style.width = "0px";
		...
		for(let idx=0; idx<json.length; idx++){
			...
			for(var i=0, u=0, w=0; i < this.colSize; i+=1){
				var titleStr;
				let bd_td = document.createElement('td'), 
					spn = document.createElement('span'),
					val = null, convVal, columnUse = true,
				...
				if(column[i].unused || !columnUse){
					...
					if(setting.option.unit.use){
						...
						if(typeof column[i].unit != "undefined"){
							...
							if(unit === ""){
								unitVal = setting.option.unit.basic || setting.option.unit.type[0];
							}else{
								unitVal = unit;
							}
						}	
						...
						if(unitVal != ""){
							var func = setting.option.unit.func;
...
$.fn.seGrid = function (data) {
		...
		let	$gridArea = this, // grid area div
			setting = $.extend(true,{}, default_option, data);
		
	//	let fGrid = gridImpl(fui.grid($gridArea, setting));
		let fGrid = new SEGrid($gridArea, setting)
		...
		return ({
			...
			//	fui.loading($gridArea);
				...
				//	$.ajaxSettings.traditional = true;  
					...
					if(jList != 0){
						append_data.list = jList.slice(0, setting.data.listCnt);
					}
					append_data.allListCnt = allCnt;
					
					fui_obj.gridAppend(append_data);
				}
